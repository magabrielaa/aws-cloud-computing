...
def create_annotation_job_request():
  ...
  # (4) Publish a notification message to SNS job requests topic
...
def annotation_details(id):
  ...
  # Save user role to pass it to the annotation.html template
  try:
    profile = get_profile(identity_id=user_id)
  except Exception as e:
    app.logger.error(f'Unable to get user profile {e}')
  ...
  # Create show_upgrade variable to pass to annotation.html render
    ...
    try:
      ...
      results_url = s3_client.generate_presigned_url(ClientMethod='get_object',
                                                    ...
                                                    Params={'Bucket': results_bucket,
                                                          'Key': results_key,
                                                          'ResponseContentDisposition': 'attachment'})
      return render_template('annotation.html', job=job_item, input_url=input_url, results_url=results_url, upgrade=show_upgrade)
    except ClientError as e:
